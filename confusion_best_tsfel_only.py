"""TSFEL-only baseline confusion matrix (without circadian features).

Same configuration as confusion_best.py but the per-subject feature vector
contains ONLY the 1092 TSFEL distributional summaries (no circadian).

Run from depresjon-clean project root:
    python confusion_best_tsfel_only.py --data-dir data/depresjon --cache-dir cache
"""
import argparse
import sys
...
def main():
    ...
    # KEY DIFFERENCE: raw_activity=None disables circadian feature integration
    ...
    print(f"Feature dimension: {X.shape[1]} (TSFEL-only, no circadian)")
    ...
    print(f"\n=== TSFEL-only baseline: {SELECTOR} + {CLASSIFIER} (LOSO, n={len(y)}) ===")
    ...
    print("\n--- Paste-ready values for TSFEL-only baseline ---")
    print(f"N_D_correct = {TP}   SENS = {sens:.3f}")
    print(f"N_C_correct = {TN}   SPEC = {spec:.3f}")
    print(f"N_D_miss (FN) = {FN}")
    print(f"N_C_miss (FP) = {FP}")
    print(f"kappa = {kappa:.3f}")
    print(f"feature_dim = {X.shape[1]}")
    print(f"median_k = {int(np.median(ks))}")
